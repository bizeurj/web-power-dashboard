...
export function PaidTab({ snapshot }: { snapshot: Snapshot }) {
  ...
  const adsCampaigns = asArray<CampaignRow>(ads.campaigns);
  const adsKeywords = asArray<KeywordRow>(ads.keywords);
  const adsByNetwork = asArray<NetworkRow>(ads.byNetwork);
  const adsDaily = asArray<number>(ads.daily);
  const adsYtd = asArray<{ yearMonth: string; cost: number; sessions: number }>(ads.ytd);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <Card title="30-day spend trend">
        <LineSeries
          labels={adsDaily.map((_, i) => `D${i + 1}`)}
          series={[{ name: 'Daily spend', data: adsDaily, color: PALETTE.gold, fill: true }]}
          height={220}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14 }}>
        <Card title="Top campaigns">
          <DataTable columns={campaignCols} rows={adsCampaigns} defaultSortKey="cost" pageSize={10} />
          {adsCampaigns.length > 0 && (
            <Insight tone="warn" icon="!">
              <strong>{adsCampaigns[0].campaign}</strong> accounts for{' '}
              {fmtPct(adsCampaigns[0].cost / Math.max(1, ads.headline.cost))} of paid spend in this window.
            </Insight>
          )}
        </Card>

        <Card title="By network">
          <DataTable columns={networkCols} rows={adsByNetwork} defaultSortKey="cost" />
        </Card>
      </div>

      <Card title="Top keywords">
        <DataTable columns={keywordCols} rows={adsKeywords} defaultSortKey="cost" pageSize={15} />
      </Card>

      {adsYtd.length > 0 && (
        <Card title="YTD monthly trend">
          <BarSeries
            labels={adsYtd.map((m) => m.yearMonth)}
            series={[
              { name: 'Cost', data: adsYtd.map((m) => m.cost), color: PALETTE.gold },
              { name: 'Sessions', data: adsYtd.map((m) => m.sessions), color: PALETTE.primary },
            ]}
            height={240}
            yFormatter={(n) => fmt(n)}
          />
